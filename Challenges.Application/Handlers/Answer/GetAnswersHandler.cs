...
public class GetAnswersHandler : ICommandHandler<GetAnswersCommand,GetAnswersResponse>
{
    ...
    public async Task<GetAnswersResponse> ExecuteAsync(GetAnswersCommand command, CancellationToken ct)
    {
        ...
        if(command.QuestionId is not null) answers = await _answerService.GetAnswersByQuestionId(command.QuestionId.Value);
        if(command.AnswerId is null && command.QuestionId is null) return new GetAnswersResponse(new Result(false, null, null, 400, "QuestionId or AnswerId must be provided"));
        if (command.CreatedBy is not null) answers = await _answerService.GetAnswersByCreatedBy(command.CreatedBy.Value);
